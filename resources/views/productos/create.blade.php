...
<head>
    ...
    <title>Crear Producto - VentasPro</title>
    ...
    <main class="pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div class="bg-white/5 backdrop-blur-lg rounded-xl p-6 border border-white/10 shadow-xl hover-scale animate-fade-in">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('productos.index') }}" class="inline-flex items-center px-4 py-2 bg-white/10 text-white rounded-lg hover:bg-white/20 transition-all duration-300">
            ...
            <form method="POST" action="{{ route('productos.store') }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Categoría -->
                    <div class="space-y-2">
                        <label for="categoria_id" class="block text-sm font-medium text-gray-300">
                            Categoría
                        </label>
                    ...
                    <!-- Precio -->
                    <div class="space-y-2">
                        <label for="precio" class="block text-sm font-medium text-gray-300">
                            Precio
                        </label>
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400">$</span>
                            <input type="number" name="precio" id="precio" value="{{ old('precio') }}" step="0.01" min="0" required
                                   class="w-full bg-white/5 border border-white/10 rounded-lg pl-8 pr-4 py-2 text-white focus:outline-none focus:border-[#FF6B3C] focus:ring-1 focus:ring-[#FF6B3C]"
                                   placeholder="0.00">
                        </div>
                    </div>
                ...
                <div class="flex justify-end space-x-4">
                    <button type="button" onclick="window.location.href='{{ route('productos.index') }}'"
