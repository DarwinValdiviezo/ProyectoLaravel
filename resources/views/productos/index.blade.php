...
<head>
    ...
    <title>Productos - VentasPro</title>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="text-left border-b border-white/10">
                            ...
                            <th class="pb-3 font-semibold text-gray-300">Categoría</th>
                            <th class="pb-3 font-semibold text-gray-300">Precio</th>
                            <th class="pb-3 font-semibold text-gray-300">Stock</th>
                    ...
                    <tbody class="divide-y divide-white/10">
                        @foreach($productos as $producto)
                            <tr class="hover:bg-white/5 transition-colors duration-200">
                                <td class="py-4">{{ $producto->nombre }}</td>
                                <td class="py-4">
                                    <span class="px-3 py-1 text-sm rounded-full bg-white/10">
                                        {{ $producto->categoria->nombre }}
                                    </span>
                                </td>
                                <td class="py-4">${{ number_format($producto->precio, 2) }}</td>
                                <td class="py-4">
                                    <span class="px-3 py-1 text-sm rounded-full {{ $producto->stock > 0 ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400' }}">
                                        {{ $producto->stock }}
                                    </span>
                                </td>
                                <td class="py-4 text-right space-x-2">
                                    ...
                                    <button onclick="openModal('delete-producto-{{ $producto->id }}-modal')"
                            ...
                            <x-confirm-modal 
                                id="delete-producto-{{ $producto->id }}-modal"
                                title="Eliminar Producto"
                                message="¿Está seguro que desea eliminar el producto {{ $producto->nombre }}? Esta acción no se puede deshacer."
                            >
                                <form action="{{ route('productos.destroy', $producto) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </x-confirm-modal>
