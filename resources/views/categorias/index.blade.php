...
<head>
    ...
    <title>Categorías - VentasPro</title>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="text-left border-b border-white/10">
                            ...
                            <th class="pb-3 font-semibold text-gray-300">Descripción</th>
                        ...
                        @foreach($categorias as $categoria)
                            <tr class="hover:bg-white/5 transition-colors duration-200">
                                ...
                                <td class="py-4 text-right space-x-2">
                                    ...
                                    <button onclick="openModal('delete-categoria-{{ $categoria->id }}-modal')"
                            ...
                            <x-confirm-modal 
                                id="delete-categoria-{{ $categoria->id }}-modal"
                                title="Eliminar Categoría"
                                message="¿Está seguro que desea eliminar la categoría {{ $categoria->nombre }}? Esta acción no se puede deshacer."
                            >
                                <form action="{{ route('categorias.destroy', $categoria) }}" method="POST" class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </x-confirm-modal>
